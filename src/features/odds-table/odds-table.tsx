...
const applyPredicates = (
    ...
    const compiled = entries.map(([label, raw]) => {
        ...
        return (row: Match) => {
            ...
            if (label === RusMatchKeys[MatchKeys.TEAMS]) {
                const _val = val.replace(/toto <div class=['"]toto['"]>.*?<\/div>/g, "");
                const [team1, team2] = _val.split("<br/>").map(s => s.trim());

                if(q.includes(team1) && q.includes(team2)) {
                    if(q === `${team1}${team2}`) {
                        return true;
                    }
                    if(q === `${team1} ${team2}` || q === `${team2} ${team1}`) {
                        return true;
                    }
                    return false;
                } else {
                    return _val.includes(q);
                }
            } else if (numericColumnHeaders.has(label)) {
            ...
            <div className="bg-white/5 backdrop-blur-xl overflow-hidden flex flex-col h-full">
                ...
                <div className="flex-1 overflow-hidden" ref={tableAreaRef}>
                    <div className="max-h-[100%] overflow-x-auto overflow-y-auto border border-slate-700 rounded-lg">
                        <table
                            className="w-full table-fixed border-collapse overflow-auto h-full min-w-[1200px] sm:min-w-[1000px]">
                                    ...
                                    {hg.headers.map((h) => (
                                        ...
                                        >
                                            <div className="relative">
                                                <div
                                                    className="select-none text-blue-300 font-bold mb-1 text-center text-xs">
                                                    {h.column.id}
                                                </div>
                                        ...
                                        {match.getVisibleCells().map((cell) => (
                                            <td
                                                ...
                                                onClick={() => {
                                                    ...
                                                    let cleanValue = renderClean(value);
                                                    cleanValue = cleanValue.replace(/toto <div class=['"]toto['"]>.*?<\/div>/g, "");
                                                    cleanValue = cleanValue.replace(/<br\/>/g, "")
                                                ...
                                                {cell.column.id === "Сигнатуры" ? (
                                                    ...
                                                    <div>
                                                        {(() => {
                                                            ...
                                                            return (
                                                                <span
                                                                    className={betResult === "win" ? "text-green-300 font-semibold" : "" + 'relative'}
                                                                    dangerouslySetInnerHTML={{
                                                                        __html: renderClean(String(cell.getValue() ?? "")),
                                                                    }}
                                                                />
                                                            )
                                                        })()}
